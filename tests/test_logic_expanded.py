...
def test_ammonia_toxic():
    tank = {
        "data": [{"Ammonia (ppm)": "0.5"}],
        "equipment": [],
        "maintenance": []
    }
    suggestions = suggest_maintenance(tank)
    assert any("ammonia" in s.lower() for s in suggestions)

def test_phosphate_elevated():
    tank = {
        "data": [{"Phosphate (ppm)": "0.2"}],
        "equipment": [],
        "maintenance": []
    }
    suggestions = suggest_maintenance(tank)
    assert any("phosphate" in s.lower() for s in suggestions)

def test_invalid_nitrate_value():
    tank = {
        "data": [{"Nitrate (ppm)": "bad_data"}],
        "equipment": [],
        "maintenance": []
    }
    suggestions = suggest_maintenance(tank)
